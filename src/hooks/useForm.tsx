...
export const useForm = (
  ...
  specificValidationFields: any = undefined,
) => {
    ...
    (name: any, value: any) => {
      ...
      if (is_object(field["validator"]) && error === "") {
        if (value && value.length > 0) {
          const fieldValidator = field["validator"] as Record<string, unknown>;
          const testFunc = fieldValidator["func"] as ((v: unknown, all: unknown) => boolean) | undefined;
          if (testFunc) {
            if (!testFunc(value, values)) {
              error =
                "convertToLang" in state?.[name]
                  ? t(fieldValidator["error"] as string)
                  : (fieldValidator["error"] as string);
            }
          } else {
            error =
              "convertToLang" in state?.[name]
  ...
  const validateMicroSiteFormFields = (name: any, value: any, type: any) => {
    ...
    const isFieldToConsider = (name: any) => {
      if (specificValidationFields.length === 0) return true;
        ...
        (specificValidationFields as string[]).includes(String(name))
      )
        return true;
      else return false;
    };
            ...
            if (field.type === "TELEPHONE_WITH_EXT") {
              if (Array.isArray(value)) {
                ...
                if (!testFunc(newVal, values)) {
                  error = fieldValidator["error"] as string;
                }
  ...
  const handleOnEditorChange = (e: any, id: any) => {
    ...
    const error = "validateFormFields(name, value)" as string;
    if (value !== null || value !== undefined) {
      setValues((prevState: any) => ({ ...prevState, [name]: value }));
      setErrors((prevState: any) => ({ ...prevState, [name]: error }));
      setDirty((prevState: any) => ({ ...prevState, [name]: true }));
  ...
  const handleOnSubmit = useCallback(
    (event?: any, extraParams?: any) => {
      ...
      if (!validateErrorState()) {
        ...
        Object.keys(errors).forEach((name) => {
          const errVal = errors[name] as string | undefined;
          if (errVal && errVal.length > 0) {
  ...
  const addUseFormFields = (fields: {
    ...
    newSchemaFields?: Record<string, unknown> | null;
  }) => {
    const { keys = [], errorMessages = [], newSchemaFields = null } = fields;
    let valuesToSet: Record<string, unknown> = {};
    let errorsToSet: Record<string, unknown> = {};
    let dirtyToSet: Record<string, unknown> = {};
    keys.forEach((eleKey: string, index: number) => {
    ...
    if (newSchemaFields !== null) {
      const clonedStates = { ...cloneDeep(state), ...newSchemaFields };
