...
const StyledInputLabel = styled.label<{ $isArabic?: boolean }>`
  display: flex;
  font-size: 14px;
  font-weight: 600;
...
const InputWrapper = styled.div<{
  ...
  &::placeholder {
    color: ${({ $placeHolderColor }) =>
      $placeHolderColor || c.PLACEHOLDER_GREY};
    opacity: 50%;
...
const StyledInput = styled.input<{
  ...
  &::placeholder {
    color: ${({ $placeHolderColor }) => $placeHolderColor || "#B5B5B5"};
...
const StyledInputWrapper = styled.div<{ $isArabic?: boolean }>`
  display: flex;
